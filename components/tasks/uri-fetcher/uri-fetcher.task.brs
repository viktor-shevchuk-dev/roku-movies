    ...
    if type(context) = "roSGNode"
      ...
      if type(parameters) = "roAssociativeArray"
        ...
        if type(uri) = "roString"
          ...
          m.urlTransferPool.Peek().setCertificatesFile("common:/certs/ca-bundle.crt")
          developerID = createObject("roAppInfo").getValue("developer_ID").toStr()
          m.urlTransferPool.Peek().addHeader("X-Roku-Reserved-Dev-Id", developerID)
          m.urlTransferPool.Peek().initClientCertificates()
          m.urlTransferPool.Peek().retainBodyOnError(true)
